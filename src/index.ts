 ...
 * - Duration string (default: '10m'): e.g., '10m', '1h', '30s' - container sleeps after this period of inactivity
 * - 'never': Container stays alive indefinitely (high cost, use only if cold starts are unacceptable)
 *
 * To change the sleep duration:
 *   npx wrangler secret put SANDBOX_SLEEP_AFTER
 *   # Enter: 10m (or 1h, 30m, never, etc.)
 */
function buildSandboxOptions(env: MoltbotEnv): SandboxOptions {
  const sleepAfter = env.SANDBOX_SLEEP_AFTER?.toLowerCase() || '10m';
